...
export default function MenusDelDiaCarousel() {
  ...
  if (loading) return <div className="h-[600px] bg-[#f5ede0] rounded-3xl animate-pulse" />

  const gradients = [
    "linear-gradient(180deg, #537895 0%, #6991C7 100%)",
    "linear-gradient(180deg, #a35c78 0%, #d65a5a 100%)",
    "linear-gradient(180deg, #9A7B4F 0%, #C9A96E 100%)",
  ]

  return (
    <div className="mb-16 relative rounded-3xl overflow-hidden py-20 min-h-[650px] flex flex-col items-center justify-center" style={{ background: "#f5ede0" }}>
      <canvas ref={canvasRef} className="absolute inset-0 pointer-events-none" />

      <div className="relative z-10 w-full">
        <h2 className="font-serif text-4xl text-stone-800 text-center mb-20 tracking-tighter">Menu del día</h2>
                      ...
                      <div className="p-8 flex-1 flex flex-col justify-center space-y-6">
                        {menu.items.map(item => (
                          <div key={item.id} className="text-center">
                            ...
                            <p className="text-xs text-orange-600 font-black mt-2">${item.price}</p>
                          </div>
                        ))}
                      </div>
                    </div>
